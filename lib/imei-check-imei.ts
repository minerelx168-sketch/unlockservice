...
const AAD = Buffer.from('iunlockmobile:imei-check-imei:v1', 'utf8')
const DEVELOPMENT_SECRET = 'local-imei-check-fingerprint-v1'

type EncryptedImeiCheckImei = {
  v: 1
  iv: string
  tag: string
  data: string
...
function rootSecret() {
  ...
  if (process.env.NODE_ENV === 'production') throw new Error('IMEI check encryption is unavailable.')
...
function encryptionKey() {
  return Buffer.from(
    hkdfSync(
      ...
      Buffer.from('iunlockmobile-imei-check-encryption-v1', 'utf8'),
...
export function encryptImeiCheckImei(value: string) {
  ...
  const envelope: EncryptedImeiCheckImei = {
    v: VERSION,
...
export function decryptImeiCheckImei(value: string | null | undefined) {
  if (!value) return null
  try {
    const parsed = JSON.parse(value) as Partial<EncryptedImeiCheckImei>
