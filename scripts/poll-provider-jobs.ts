...
async function main() {
  ...
  process.stdout.write(`${JSON.stringify({ provider, payments, notifications })}\n`)
  if (provider.errors > 0 || payments.errors > 0 || notifications.failed > 0 || notifications.leaseLost > 0) {
    process.exitCode = 1
  }
}
